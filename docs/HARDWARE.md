...
- **Deep sleep:** After 3 min of screen-off with no active playback (5 min total from last input), the firmware enters ESP32 deep sleep (<1 mA). The CENTER button (GPIO 4, EXT0 wakeup) wakes the device. GPIO 7 (backlight) is held LOW during sleep; a hardware pull-down on the backlight transistor base is recommended on custom PCBs to ensure the backlight stays off if the GPIO floats. For next PCB revision, move backlight to an RTC GPIO (e.g. GPIO 27) for reliable `rtc_gpio_hold_en` support.
